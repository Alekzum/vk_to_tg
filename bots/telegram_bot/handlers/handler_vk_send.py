...
from .handler_vk import CHATS_PAGE_LEN, HandlerVKThings
...
async def get_start_data(event: ChatEvent, manager: DialogManager):
...
async def on_send_message(manager: DialogManager, **kwargs) -> dict[str, str]:
    user: types.User = manager.middleware_data["user"]

    api = await get_vk_api(user.id)
    # print("dialog_data", dialog_manager_dialog_data=manager.dialog_data)
    photo = manager.dialog_data.get("msg_photo", None)
    text = manager.dialog_data["msg_text"]
    chat_name = manager.dialog_data["msg_name"]
    result = dict(msg_text=text, msg_chat=chat_name)

    if photo:
        result["msg_photo"] = photo

    reply_message = manager.dialog_data.get("msg_to", None)
    if not reply_message:
        return result

    rtm = VkMessage.model_validate(
        (await api.messages.getById(message_ids=reply_message))["items"][0]
    )
    message_text = (await parse_message(rtm, api, 0))[0]
...
#     await manager.switch_to(BotStates.Answer.CHOOSE_CHAT)
...
async def on_chat_selected(
    ...
    chats_list = manager.dialog_data["vk_chat_list_data"]
    ...
    manager.dialog_data["msg_in"] = selected_chat_id
    manager.dialog_data["msg_name"] = selected_chat_name
    manager.dialog_data["msg_to"] = None
    await manager.switch_to(VKBotStates.Answer.SEND_MESSAGE)
    ...
    async with asyncio.TaskGroup() as group:
        for chat in raw_chats.response.items:
            ...
            task = group.create_task(
                get_dialog_name(peer_id=peer_id, api=api, dialog_type=peer_type)
            )
...
async def chats_getter(
    ...
    if not raw_chats:
        ...
        try:
            ...
            logger.warning(
                "Didn't got chats bc exception", user_id=user_id, exc_info=True
            )
...
async def on_page_changed(
    event: ChatEvent, widget: ManagedScroll, manager: DialogManager
):
    page = await widget.get_page()
    manager.dialog_data["vk_chat_list_offset"] = page * CHATS_PAGE_LEN


def clear_reply_data(manager: DialogManager):
    for key in [
        "msg_act",
        "msg_type",
        "msg_text",
...
send_message_dialog = Dialog(
    Window(
        ...
        *HandlerVKThings.maybe_inputs,
        ...
        state=VKBotStates.Answer.SEND_MESSAGE,
    ...
    Window(
        ...
        state=VKBotStates.Answer.CHOOSE_CHAT,
        getter=chats_getter,
    ...
    Window(
        ...
        state=VKBotStates.Answer.MAYBE_CHOOSE_CHAT,
    ),
